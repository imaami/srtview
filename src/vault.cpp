...
// "<hex>.txt" and "<hex>.<suffix>.txt", never the tmp -- sorted so
// adoption picks deterministically.
                                         ...
                                         agenda::kind k) const
{
	...
	std::error_code ec;
	fs::directory_iterator it(m_dir + '/' + std::string(sub(k)),
	                          ec);
	for (fs::directory_iterator const end; !ec && it != end;
	     it.increment(ec)) {
		std::string n = it->path().filename().string();
		if (n.size() > hex.size() && n.starts_with(hex)
		    && n[hex.size()] == '.' && n.ends_with(".txt"))
...
std::string store::place(agenda::task const &t)
{
	...
	// Fresh bytes are about to land: dependents resolving later
	// must hash them, not a memo of the old artifact.
	m_entries[at].bytes = {};
	m_entries[at].resolved = true;
	m_entries[at].path = target;
	return target;
