...
- Controllers/mediators: `PlaybackCtl` (`playback.cpp`) executes transport verbs against mpv; `SearchCtl` (`search.cpp`) owns pattern semantics and match navigation; `Refinery` (`refinery.cpp/hpp`) owns every machine that turns landed model artifacts into corpus adoptions — term windows, spelling votes, the directory fold, topic dives, probe/focus chains — plus the completion-poked dispatch driving them: Facts pokes on landing, one queued wake per burst runs the chain (`landed()`-swept, so idle costs nothing), and the UI is reached only through `refinery_host` (one something-changed report). The term directory is a *projection*: landed replies parse once into per-window facts, and `refoldTerms()` rebuilds the directory from scratch in window staging order — then walks the merge/spell chain to fixpoint from cached verdicts (ask ids hash sorted term titles and evidence bodies, never termN names), and derives dives from the final directory. Names, first-nonempty fields and fold outcomes are a pure function of (cut, artifacts, the non-term topic set) — arrival order can't reorder them, and a stale generation's replies exclude themselves because the fold reads only the current cut's windows; no adoption gate exists outside the plan — asks admit through `plan::ready()`, artifact reads through `plan::complete()` (done with every gate recursively open), and the frame-sensitive asks behind these facts carry their cut's ground witness in `deps`, which holds a cached reply exactly as it holds the ask. The focus chain folds the same way: probe/focus pair ids are content-addressed (sorted dive-pattern hashes), the staged-scan sequence fixes the pair order, and focusN adoption replays in it from per-pair facts — only adhocN stays event-ordered, being user action.
